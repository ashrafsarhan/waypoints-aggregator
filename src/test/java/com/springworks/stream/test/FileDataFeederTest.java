...
public class FileDataFeederTest {
	...
	@Test
	public void test() {
		dataFeed.streamDataEvent().map(e -> e).forEach(System.out::println);
		//Assert the stream size (file lines count)
		assertEquals("Test Fails (Not the expected events counts)", 5, dataFeed.streamDataEvent().count());
		//Assert on the first data event timestamp (first line in the file)
		assertEquals("Test Fails (Not the expected first event)", "2016-06-21T12:00:00.000Z", dataFeed.streamDataEvent().map(e -> e).findFirst().get().getTimestamp());	
	}
